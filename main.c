...
static void OrdenarRanking(void)
{
    ...
    while (ranking)
    {
        NoRank *c = ranking;
        ranking = c->prox;
        if (!ord || (c->gf - c->ga) > (ord->gf - ord->ga))
        {
            c->prox = ord;
            ord = c;
        }
        ...
        {
            NoRank *p = ord;
            while (p->prox && (p->prox->gf - p->prox->ga) >= (c->gf - c->ga))
                p = p->prox;
            c->prox = p->prox;
            p->prox = c;
        }
...
static void SalvarRanking(const char *arq)
{
    ...
    if (!f)
        return;
    for (NoRank *n = ranking; n; n = n->prox)
...
static void AtualizarJogo(float dt, Tela *tela)
{
    ...
    if (fabsf(alvoX - (ia.caixa.x + LARG_JOG / 2)) < 10)
        dir = 0;
    ia.vel.x = dir * velBaseIA;
    if (bola.pos.y < ia.caixa.y && ia.vel.y == 0)
        ia.vel.y = -VEL_PULO * 0.9f;
    ...
    /* quique chão/paredes */
    if (bola.pos.y > CHAO_Y - RAIO_BOLA)
    {
        bola.pos.y = CHAO_Y - RAIO_BOLA;
        bola.vel.y *= -RESTITUIÇÃO;
    }
    if (bola.pos.x < RAIO_BOLA)
    {
        bola.pos.x = RAIO_BOLA;
        bola.vel.x *= -RESTITUIÇÃO;
    }
    if (bola.pos.x > LARG_JAN - RAIO_BOLA)
    {
        bola.pos.x = LARG_JAN - RAIO_BOLA;
        bola.vel.x *= -RESTITUIÇÃO;
    }
    ...
    if (CheckCollisionCircleRec(bola.pos, RAIO_BOLA, golDir))
        MarcarGol(1);
    if (CheckCollisionCircleRec(bola.pos, RAIO_BOLA, golEsq))
        MarcarGol(0);
...
{
    if (jogOK)
        DrawTexture(texJog, jogador.caixa.x, jogador.caixa.y, WHITE);
    else
        DrawRectangleRec(jogador.caixa, BLUE);

    if (iaOK)
        DrawTexture(texIA, ia.caixa.x, ia.caixa.y, WHITE);
    else
        DrawRectangleRec(ia.caixa, RED);
...
static void DesenharRanking(void)
{
    ...
    {
        char d[32];
        strftime(d, sizeof d, "%d/%m %H:%M", localtime(&n->ts));
        TextoSombra(TextFormat("%02d. %-8s  %d x %d  %s",
                               pos, n->nome, n->gf, n->ga, d),
                    140, y, 24, WHITE);
    }
...
int main(void)
{
    ...
    texFundo = LoadTexture("assets/recife_bg.jpeg");
    fundoOK = texFundo.id != 0;
    texJog = LoadTexture("assets/player1.png");
    jogOK = texJog.id != 0;
    texIA = LoadTexture("assets/player2.png");
    iaOK = texIA.id != 0;
    texBola = LoadTexture("assets/bola.png");
    bolaOK = texBola.id != 0;
    ...
    if (jogOK)
        UnloadTexture(texJog);
    if (iaOK)
        UnloadTexture(texIA);
    if (bolaOK)
        UnloadTexture(texBola);
